...
import { languagesTranslationKeys, levelsTranslationKeys, radioButtonsTranslationKeys } from '~/containers/find-offer/offer-filter-block/offer-filter-list/OfferFilterList.constants'
import { styles } from '~/containers/find-offer/offer-filter-block/offer-filter-list/OfferFilterList.styles'
import { FindOfferFilterTypes, FindOffersFilters, FindOffersUpdateFilter } from '~/types'
...
const OfferFilterList:FC<OfferFilterListProps> = ({ updateFilter, updateFilterByKey, filters }) => {
  const { t } = useTranslation()
  const languageOptions = languagesTranslationKeys.map(language => t(language))
  const levelOptions = levelsTranslationKeys.map(radio => t(radio))
  const radioOptions = radioButtonsTranslationKeys.map(({ title, value }) => ({ title: t(title), value }))
