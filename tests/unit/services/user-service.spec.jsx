...
describe('userService tests', () => {
  ...
  it('should make a GET request to the correct URL', async () => {
    ...
    mockAxiosClient
      ...
      .replyOnce(200, { items: [], count: 0 })

    await userService.getUsers(options)

    expect(mockAxiosClient.history.get[0].url).toBe(
      `${URLs.users.get}?limit=${options.limit}&skip=${options.skip}`
    )
  })
  it('should return an array of users', async () => {
    const users = [userDataMock]
    mockAxiosClient.onGet(new RegExp(URLs.users.get)).replyOnce(200, users)
  ...
  it('should return a user by userId and userRole', async () => {
    ...
    const result = await userService.getUserByIdWithBaseService(
      userId,
      userRole
    )
  ...
  it('should delete a user by userId', async () => {
    mockAxiosClient
      .onDelete(URLs.users.delete.replace(':id', userId))
      .reply(204, null)
  ...
  it('should delete multiple users by userIds', async () => {
    mockAxiosClient.onPost(URLs.users.deleteMany, userIds).reply(204, null)
