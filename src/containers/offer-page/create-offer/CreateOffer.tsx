...
const CreateOffer:FC<CreateOfferProps> = ({ closeDrawer }) => {
  ...
  const { data, errors, isDirty, handleInputChange, handleNonInputValueChange, handleBlur, handleSubmit } = useForm<CreateOfferData>(
    { initialValues,
      validations,
      onSubmit: async () => {
        await createOffer()
  ...
  const handleAutocompleteChange = (key: keyof CreateOfferData ) => (_: React.ChangeEvent<HTMLInputElement>, value: string | null) => {
    if(key === 'languages'){
      value && !data.languages.includes(value) && handleNonInputValueChange(key, [...data.languages, value])
    }
    else{
